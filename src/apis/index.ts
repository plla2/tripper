...
export const getMainFestivalItems = async (
  ...
  try {
    const res = await axios.get(
      ...
      }/searchFestival1?numOfRows=${numOfRows}&pageNo=1&MobileOS=ETC&MobileApp=tripper&_type=json&areaCode=${areaCode}&arrange=Q&eventStartDate=${startDate}&serviceKey=${
        import.meta.env.VITE_API_KEY
      } `
    );
    return res.data.response.body.items.item;
